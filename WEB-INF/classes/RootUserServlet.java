...
public class RootUserServlet extends HttpServlet {
	...
	public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
        	...
        	boolean inShipment = false;
        	
        	isSelect = sqlStatement.toUpperCase().trim().contains("SELECT");
        	
        	inShipment = sqlStatement.toUpperCase().trim().contains("SHIPMENTS");
        	...
        	if (isSelect) {
        		...
        		if (inShipment) { // Checks for business logic on shipment inserts.
        			
        			mysqlUpdateValue = statement.executeUpdate(sqlStatement);
        			
        			int supplierUpdates = statement.executeUpdate("update suppliers set status = status + 5 where snum in (select snum from shipments where quantity >= 100);");
        			
        			message = "<tr><th>The statement executed successfully. A total of " + mysqlUpdateValue + " row(s) were updated.<br>Business Logic Detected! - <br>Business Logic updated " 
        					+ supplierUpdates + " Updating Supplier Status marks.</th></tr>";
        		}
        		else { // Regular user update.
